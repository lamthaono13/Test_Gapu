...
namespace PFramework
{
    public static class Messenger
    {
        ...
        static void OnListenerAdding(int eventIndex, Delegate listenerBeingAdded)
        {
            ...
            {
                Debug.LogError(String.Format("Attempting to add listener with inconsistent signature for event type {0}. Current listeners have type {1} and listener being added has type {2}", (GameEvent)eventIndex, d.GetType().Name, listenerBeingAdded.GetType().Name) );
            }
        }
        ...
        static void OnListenerRemoving(int eventIndex, Delegate listenerBeingRemoved)
        {
            ...
            if (d == null)
            {
                Debug.LogWarning(String.Format("Attempting to remove listener with for event type \"{0}\" but current listener is null.", (GameEvent)eventIndex));
            }
            else if (d.GetType() != listenerBeingRemoved.GetType())
            {
                Debug.LogError(String.Format("Attempting to remove listener with inconsistent signature for event type {0}. Current listeners have type {1} and listener being removed has type {2}", (GameEvent)eventIndex, d.GetType().Name, listenerBeingRemoved.GetType().Name));
            }
        }

        static void OnBroadcasting(int eventIndex, Delegate broadcastMessage)
        {
            Debug.LogError(String.Format("Broadcasting message \"{0}\" but listeners have a different signature than the broadcaster.", (int)eventIndex));
        }
        ...
        //No parameters
        static public void AddListener(GameEvent gameEvent, Callback handler)
        {
        ...
        //Single parameter
        static public void AddListener<T>(GameEvent gameEvent, Callback<T> handler)
        {
            int index = (int)gameEvent;
        ...
        //Two parameters
        static public void AddListener<T, U>(GameEvent gameEvent, Callback<T, U> handler)
        {
            int index = (int)gameEvent;
        ...
        //Three parameters
        static public void AddListener<T, U, V>(GameEvent gameEvent, Callback<T, U, V> handler)
        {
            int index = (int)gameEvent;
